...
const FROM = process.env.EMAIL_FROM ?? "NEO Marketing Hub <notifications@neohomeloans.com>";
...
export interface MentionEmailPayload {
  ...
  actorName: string;
  entityLabel: string;  // "a task", "a meeting", "a Rock", etc.
  commentBody: string;
  link: string;         // relative path like /meetings/abc123
}
...
export async function sendMentionEmail(payload: MentionEmailPayload) {
  ...
  // Highlight @mentions in the comment body
...
<body style="margin:0;padding:0;background:#061320;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
    ...
    <tr>
      <td align="center">
        <table width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;">
          ...
          <!-- Header -->
          <tr>
            <td style="background:#0e2b48;border-radius:12px 12px 0 0;padding:24px 28px;border-bottom:1px solid #1d4368;">
          ...
          <!-- Body -->
          <tr>
            <td style="background:#0b1f35;padding:28px;border-left:1px solid #1d4368;border-right:1px solid #1d4368;">
              <p style="margin:0 0 20px;font-size:15px;color:#e2e8f0;line-height:1.5;">
          ...
          <!-- Footer -->
          <tr>
            <td style="background:#061320;border-radius:0 0 12px 12px;padding:18px 28px;border:1px solid #1d4368;border-top:none;">
  ...
  try {
    ...
    // Don't let email failure break the comment flow
